...
trait Step5_PrimaryPersistenceSpec { this: KVStoreSuite =>
  ...
  @Test def `Step5-case1: Primary does not acknowledge updates which have not been persisted`()
    : Unit = {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(
      Replica.props(arbiter.ref, probeProps(persistence)),
      "step5-case1-primary"
    )
  ...
  @Test def `Step5-case2: Primary retries persistence every 100 milliseconds`()
    : Unit = {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(
    ...
    // Retries form above
    persistence.expectMsg(
      200.milliseconds,
      Persist("foo", Some("bar"), persistId)
    )
    persistence.expectMsg(
      200.milliseconds,
      Persist("foo", Some("bar"), persistId)
  ...
  @Test def `Step5-case3: Primary generates failure after 1 second if persistence fails`()
    : Unit = {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(
  ...
  @Test def `Step5-case4: Primary generates failure after 1 second if global acknowledgement fails`()
    : Unit = {
    val arbiter = TestProbe()
    val primary = system.actorOf(
  ...
  @Test def `Step5-case5: Primary acknowledges only after persistence and global acknowledgement`()
    : Unit = {
    val arbiter = TestProbe()
    val primary = system.actorOf(
      Replica.props(arbiter.ref, Persistence.props(flaky = false)),
    ...
    arbiter.send(
      primary,
      Replicas(Set(primary, secondaryA.ref, secondaryB.ref))
    )
